  ...
  useEffect(() => {
    ...
    const verifyAccess = async () => {
      try {
        ...
        if (!cancelled) setAccessStatus('authorized')
      } catch {
        if (!cancelled) setAccessStatus('unauthorized')
      }
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4">
        <div className="max-w-md w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg p-8 text-center space-y-4">
          ...
          <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">无访问权限</h1>
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-xl font-semibold text-gray-900 dark:text-white">源检测工具</h1>
                  <p className="text-sm text-gray-500 dark:text-gray-400">测试各个源的搜索性能和质量</p>
                </div>
              </div>
            </div>
          </div>
