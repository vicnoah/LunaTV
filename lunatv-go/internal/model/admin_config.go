...
// ---------------------------------------------------------------------------
// Go mirror of the TypeScript AdminConfig interface (admin.types.ts)
// ---------------------------------------------------------------------------
...
// Used in both per-user config (UserEmbyConfig) and the global EmbyConfigSection.
// Fields only relevant to global sources (LastSyncTime, ItemCount, IsDefault, IsPublic)
// are omitted from per-user entries at the JSON layer via omitempty.
...
// ProxyConfig is shared by TVBoxProxyConfig and VideoProxyConfig — both have
// identical shapes in the TypeScript source.
...
type TelegramAuthConfig struct {
	...
	ButtonSize         string `json:"buttonSize"` // large | medium | small
	ShowAvatar         bool   `json:"showAvatar"`
	RequestWriteAccess bool   `json:"requestWriteAccess"`
}

// OIDCAuthConfig is the legacy single-provider OIDC config (OIDCAuthConfig in TS).
// New multi-provider setup uses OIDCProviders ([]OIDCProvider).
...
// OIDCProvider extends the single-provider fields with an ID and display Name,
// allowing multiple providers to coexist.
...
// EmbyConfigSection holds global/shared Emby server sources visible to all users.
